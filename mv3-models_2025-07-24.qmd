...
- isolate final model (with pooled predictors)
...
\newpage
\FloatBarrier
# Step 1 - Building 'New' Model Based on 2023/24 Data

- using data collected in the 2023/24 academic year
- using the updated (2025) item parameters
...
for (grade in names(models)) {
  for (model in names(models[[grade]])) {
    ...
    for (method in c("Youden", "MaxSpSe", "SpEqualSe", "MaxEfficiency", "PrevalenceMatching")) {
      ...
      roc.curves.en[[model]] <- out[[1]] + labs(title = model,
                                             subtitle = paste0("risk ~ ", models[[model]]))
      proc.curves.en[[model]] <- out[[2]] + labs(title = model,
                                              subtitle = paste0("risk ~ ", models[[model]]))
      pred.dfs.en[[model]] <- df.risk.pred
      # aucs[[model]] <- mean(out[[1]]$data$AUROC)
      
      pooled.models.en[[grade]][[model]] <- results[[2]]
    }
  }
}
...
```{r}
#| label: tbl-model-eval-summary-ay2324-es
...
# Step 2 - Evaluating Model On 24/25 Data
...
#| tbl-cap: "Demographic Characteristics of the AY 24/25 Sample (N = xxxx) by Grade and Screening Language \\newline"
df.2425 %>% 
  select(c(student_id, grade, lang, gender, race, ethnicity, el, ever_disability, home_lang)) %>%
  unique() %>% 
  mutate(lang = case_when(lang == "en-US" ~ "English",
...
for (grd in names(models)) {
  for (mdl in names(models[[grd]])) {
    ...
    model <- pooled.models.en[[grd]][[mdl]]
