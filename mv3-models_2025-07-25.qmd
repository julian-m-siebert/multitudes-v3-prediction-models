...
- English models are built using data collected in the *2023/24 academic year* only (sample described in left set of columns in @tbl-ay2324-sample-en); *N* = `r N.2324`, from `r n.school.2324` schools across `r n.district.2324` districts
...
- English models are evaluated using unseen data from *2024/25 academic year* (sample described in right set of columns @tbl-ay2324-sample-en); *N* = `r N.2425`, from `r n.school.2425` schools across `r n.district.2425` districts
  ...
  for (mdl in names(models[[grd]])) {
    ...
    df.out0 <- df.model %>% 
      mutate(pred.prob = df.results$pred.prob,
             ...
             risk = risk,
             risk.pred = case_when(pred.prob < cutpoint ~ 0,
...
- built using data collected in the *2023/24 and 2024/25academic year*
...
```{r}
#| label: tbl-sample-es
#| tbl-cap: "Demographic Characteristics of the Sample for the Spanish Prediciton Model (N = xxx) by Grade\\newline"
df.es %>% 
