...
```{r load-data}
df <- read_csv("data/df-mv3-prediction-models_2025-08-01.csv") %>% 
  mutate(grade = case_when(grade == "Kindergarten" ~ "K",
         ...
         lang_inst = case_when(lang_of_instruction == "English Only" ~ "English Instruction",
                               !is.na(lang_of_instruction) ~ "Bilingual Instruction"
                               )
         ) %>% 
  select(-lang_of_instruction) %>% 
...
df.en %>% 
  select(c(student_id, ay, grade, gender, race, ethnicity, el, ever_disability, home_lang, lang_inst)) %>%
  unique() %>% 
...
## LOGO Cross-validated Models (2023/24 Only)
...
"G1" = list("0. WRE only" = "wre_e",
            ...
            "3. WRE, RANO" = "wre_e + rano_e",
            "4. WRE, RANO, EVO" = "wre_e + rano_e + evo_e",
            "5. WRE, EVO" = "wre_e + evo_e"
            ),
...
for (grade in names(models)) {
  for (model in names(models[[grade]])) {
    ...
    # build model
    results <- build_prediction_model_bootstrap_cv(df.modelcomp,
                                         grd = grade,
                                         lg = lg,
    ...
    for (group in groups) {
      ...
      if (group == "All") {
        ...
        n = df.risk.pred %>% filter(!is.na(pred.risk)) %>% select(student_id) %>% unique() %>% nrow()
        ...
        n = df.risk.pred.inner %>% filter(!is.na(pred.risk)) %>% select(student_id) %>% unique() %>% nrow()
        if (n < 5) next
...
#| tbl-cap: "English-to-English Prediction Models Using Data from 2023/24 AY Only.\\newline"
...
df.eval.en.wide.2324 |> 
  ...
  mutate(across(c(contains("Accuracy"), contains("Sensitivity"), contains("Specificity")), color_format)) %>% 
  kable(format = "latex",
        escape = FALSE,
        booktabs = TRUE,
        digits = 3,
        align = c("l", "l", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c", "c"),
        col.names = c("Grade",
                      "Predictors",
                      # "Method",
                      "n",
        ...
        linesep = c(rep("", 3), "\\addlinespace", rep("", 5), "\\addlinespace", rep("", 3))
  ) |> 
  kable_styling(
    full_width = FALSE,
    latex_options = c("scale_down")
  ) |> 
  add_header_above(c(" " = 2, "All" = 7, "EL" = 7, "EO" = 7)) |> 
  add_footnote(label = "Note. green: >= .8; black: >= .7; yellow: >= .6; red otherwise", notation = "none")
```


## LOGO Cross-validated Models (2023/24 and 2024/25 Combined)
\FloatBarrier

```{r model-comparison-en-ays2325, warning=FALSE, message=FALSE}
df.modelcomp <- df.en
...
roc.curves.en.2325 <- list()
proc.curves.en.2325 <- list()
pred.dfs.en.2325 <- list()
# aucs <- list()
pooled.models.en.2325 <- list("K" = list(),
                              "G1" = list(),
                              "G2" = list()
)
opt.cuts.en.2325 <- list("K" = list(),
                         "G1" = list(),
                         "G2" = list()
)
...
dummy_cm <- confusionMatrix(factor(c(0,1,1,0)), factor(c(0,1,0,1)))
dummy_metrics <- extract_metrics(dummy_cm)
df.eval.en.2325 <- data.frame(matrix(ncol = ncol(dummy_metrics), nrow = 0))
colnames(df.eval.en.2325) <- colnames(dummy_metrics)
df.eval.en.2325 <- data.frame(grade = as.character(), 
                              model = as.character(),
                              predictors = as.character(),
                              group = as.character(),
                              n = as.numeric(),
  ...
  for (model in names(models[[grade]])) {
    ...
    for (group in groups) {
      ...
      if (group == "All") {
        ...
        df.eval.en.2325 <- df.eval.en.2325 |> 
          rbind(df.out)
    ...
    # save plots and prediction outputs for later use
    out = plot_roc_curves_within_model(df.plots)
    roc.curves.en.2325[[model]] <- out[[1]] + labs(title = model,
                                                   subtitle = paste0("risk ~ ", models[[model]]))
    proc.curves.en.2325[[model]] <- out[[2]] + labs(title = model,
                                                    subtitle = paste0("risk ~ ", models[[model]]))
    pred.dfs.en.2325[[model]] <- df.risk.pred
    # aucs[[model]] <- mean(out[[1]]$data$AUROC)
    
    pooled.models.en.2325[[grade]][[model]] <- results[[2]]
    opt.cuts.en.2325[[grade]][[model]] <- opt.cut
  }
}
...
df.eval.en.wide.2325 <- df.eval.en.2325 |> 
  pivot_wider(names_from = group, values_from = Accuracy:n) |> 
  select(c(grade, model, predictors, method, opt.cut,
           contains("_All"),
           contains("_EL"),
...
```{r}
#| label: tbl-model-eval-summary-ays2325-en
#| tbl-cap: "English-to-English Prediction Models Using Data from Both 2023/24 and 2024/25 AYs.\\newline"
...
<!-- final.models.en.2324 <- list("K" = list(), -->
<!--                              "G1" = list(), -->
<!--                              "G2" = list() -->
<!--                              ) -->
<!-- final.opt.cuts.en.2324 <- list("K" = list(), -->
<!--                                "G1" = list(), -->
<!--                                "G2" = list() -->
<!--                                ) -->
...
<!--     # build model -->
<!--     results <- train_model_final(df.modelcomp, -->
<!--                                  grd = grade, -->
<!--                                  lg = lg, -->
<!--                                  mdl = models[[grade]][[model]] -->
<!--                                  ) -->

<!--     final.models.en.2324[[grade]][[model]] <- results[[1]] -->
<!--     final.opt.cuts.en.2324[[grade]][[model]] <- results[[2]] -->
<!--   } -->
<!-- } -->
<!-- ``` -->

<!-- ```{r model-eval-en-2324} -->
<!-- # Create a dataframe to hold evaluation metrics -->
<!-- dummy_cm <- confusionMatrix(factor(c(0,1,1,0)), factor(c(0,1,0,1))) -->
<!-- dummy_metrics <- extract_metrics(dummy_cm) -->
<!-- df.eval.en.final.2324 <- data.frame(matrix(ncol = ncol(dummy_metrics), nrow = 0)) -->
<!-- colnames(df.eval.en.final.2324) <- colnames(dummy_metrics) -->
<!-- df.eval.en.final.2324 <- data.frame(grade = as.character(),  -->
<!--                               model = as.character(), -->
<!--                               # predictors = as.character(), -->
<!--                               group = as.character(), -->
<!--                               n = as.numeric(), -->
...
<!--     model <- final.models.en.2324[[grd]][[mdl]] -->
<!--     opt.cut <- final.opt.cuts.en.2324[[grd]][[mdl]] -->
...
<!--           n = df.temp.outer %>% filter(!is.na(pred.risk)) %>% select(student_id) %>% unique() %>% nrow() -->
...
<!--           # n = df.temp.inner %>% nrow() -->
<!--           n = df.temp.inner %>% filter(!is.na(pred.risk)) %>% select(student_id) %>% unique() %>% nrow() -->
<!--           if (n < 5) next -->
...
<!-- test = df.eval.en.2324 -->
<!-- df.eval.en.2324 = test -->
<!-- # re-arrange columns -->
<!-- df.eval.en.2324 <- df.eval.en.2324 |>  -->
...
<!-- ```{r} -->
<!-- #| label: tbl-model-eval-summary-finalmodels-2324-en -->
<!-- #| tbl-cap: "Final (Full Data) English-to-English Prediction Models Built Using Data from 2023/24 AY.\\newline" -->
...
## Application to MV2 data (23/24 Only)

```{r mv2-eval-en-2324}

df.eval.mv2.en <- data.frame(grade = as.character(), 
                             model = as.character(),
                             n = as.numeric(),
                             pred.prev = as.numeric()
)

groups = c("All", "EL", "EO")
lg ="English"

df.mv2 <- read_rds("../multitudes/data/df-scores-combined-2025thetas_2025-07-24.Rdata") %>% 
  filter(event == "Fall24") %>% 
  mutate(grade = case_when(grade == "Kindergarten" ~ "K",
                           grade == "Grade 1" ~ "G1",
                           grade == "Grade 2" ~ "G2",
                           ),
         task = case_when(lang == "en-US" ~ paste0(task, "_e"),
                          lang == "es-US" ~ paste0(task, "_s")
                          ),
         task = tolower(task)
         )
...
for (grd in names(models)) {
  for (mdl in names(models[[grd]])) {
    ...
    df.out0 <- df.model %>% 
      mutate(pred.prob = df.results$pred.prob,
             pred.logit = df.results$logit,
             pred.risk = case_when(pred.prob < cutpoint ~ 0,
                                   pred.prob >= cutpoint ~ 1,
                                   is.na(pred.prob) ~ NA,
                                   )
             ) %>% 
      filter(!is.na(pred.risk)) %>% 
      select(c(student_id, contains("pred"))) %>% 
      left_join(df |>
                  select(c(student_id,
                           el,
                           race,
    ...
    pred.prev = mean(df.out0$pred.risk == 1, na.rm = TRUE)
    n = length(unique(df.out0$student_id))
            
    out <- c(grd, mdl, n, pred.prev)
    
    df.eval.mv2.en <- df.eval.mv2.en |> 
      rbind(out)
  }
}
...
```{r}
#| label: tbl-model-eval-prev-mv2-en-2324
#| tbl-cap: "Predicted Prevalence of Risk in MV2 Sample (ALL Data Collected in Fall 2024) for English Models Using 23/24 Data Only.\\newline"

colnames(df.eval.mv2.en) <- c("grade", "model", "n", "P.Prev." )
df.eval.mv2.en |> 
  mutate(grade = factor(grade, levels = c("K", "G1", "G2")),
         # model = str_sub(model, 3, 100)
  ) |>
  ...
  kable(format = "latex",
        ...
        linesep = c(rep("", 3), "\\addlinespace", rep("", 5), "\\addlinespace", rep("", 5))
  ) |> 
  kable_styling(
    full_width = FALSE,
    latex_options = c("scale_down")
...
## Application to MV2 data (23/24 and 24/25 Combined)
...
for (grd in names(models)) {
  for (mdl in names(models[[grd]])) {
    ...
    model <- pooled.models.en.2325[[grd]][[mdl]]
    cutpoint <- df.eval.en.wide.2325 %>%
      filter(grade == grade,
             model == mdl
             ) %>% 
      pull(opt.cut)
...
```{r}
#| label: tbl-model-eval-prev-mv2-en-2325
#| tbl-cap: "Predicted Prevalence of Risk in MV2 Sample (ALL Data Collected in Fall 2024) for English Models Using Combined 23/24 and 24/25 Data\\newline"
...
<!-- - built using data collected in the *2023/24 and 2024/25* academic years -->
...
#| tbl-cap: "Spanish-to-Spanish Prediction Models Using Data from Both 2023/24 and 2024/25 AYs.\\newline"
...
df.eval.es.wide |> 
  ...
  kable(format = "latex",
        ...
        linesep = c(rep("", 3), "\\addlinespace", rep("", 2), "\\addlinespace", rep("", 4))
  ) |>
  kable_styling(
    full_width = FALSE,
    latex_options = c("scale_down")
  ) |>
  add_header_above(c(" " = 2, "All" = 7)) |>
  add_footnote(label = "Note. green: >= .8; black: >= .7; yellow: >= .6; red otherwise", notation = "none")
```

## Application to MV2 data (Combined 23/24 and 24/25)

```{r mv2-eval-es}

df.eval.mv2.es <- data.frame(grade = as.character(), 
                             model = as.character(),
                             n = as.numeric(),
                             pred.prev = as.numeric()
)
...
for (grd in names(models)) {
  for (mdl in names(models[[grd]])) {
    ...
    model <- pooled.models.es[[grd]][[mdl]]
    cutpoint <- df.eval.es.wide %>%
      filter(grade == grade,
             model == mdl
             ) %>% 
      pull(opt.cut)
...
```{r}
#| label: tbl-model-eval-prev-mv2-es
#| tbl-cap: "Predicted Prevalence of Risk in MV2 Sample (ALL Data Collected in Fall 2024) for Spanish Models.\\newline"

colnames(df.eval.mv2.es) <- c("grade", "model", "n", "P.Prev." )
df.eval.mv2.es |> 
  mutate(grade = factor(grade, levels = c("K", "G1", "G2")),
         model = str_sub(model, 3, 100)
  ) |>
